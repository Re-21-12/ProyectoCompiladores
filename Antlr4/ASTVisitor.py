...
class ASTVisitor(ExprVisitor):
    ...
    def visitBloque_de_sentencia(self, ctx):
        if ctx.sentencia():
            ...
            if result is not None:
                return ASTNode('Block', children=[result])
            else:
    ...
    def visitExpr(self, ctx):
        ...
        # Manejo de suma/resta encadenada
        if ctx.term() and len(ctx.term()) > 1:
            left = self.visit(ctx.term(0))
            for i in range(1, len(ctx.term())):
                op = ctx.getChild(2 * i - 1).getText()
                right = self.visit(ctx.term(i))
                left = ASTNode('BinaryOp', children=[left, right], value=op)
            return left
        # Manejo de módulo
        if ctx.term() and ctx.MODULO():
            left = self.visit(ctx.term(0))
            right = self.visit(ctx.term(1))
            return ASTNode('BinaryOp', children=[left, right], value='%')
    ...
    def visitFactor(self, ctx):
        if ctx.RAIZ():
            ...
            return ASTNode('Literal', value=ctx.CADENA().getText()[1:-1])
        elif ctx.VARIABLE() and ctx.MASMAS():
            return ASTNode('UnaryOp', children=[ASTNode('Variable', value=ctx.VARIABLE().getText())], value='++')
    ...
    def visitActualizacion(self, ctx):
        var_name = ctx.VARIABLE().getText()
        if ctx.MASMAS():
            return ASTNode('UnaryOp', children=[ASTNode('Variable', value=var_name)], value='++')
        elif ctx.MENOSMENOS():
            return ASTNode('UnaryOp', children=[ASTNode('Variable', value=var_name)], value='--')
        elif ctx.expr():
            expr = self.visit(ctx.expr())
            return ASTNode('Assignment', children=[expr], value=var_name)
        return ASTNode('Empty')
